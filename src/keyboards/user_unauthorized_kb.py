...
welcome_kb.add(KeyboardButton('\U0001f525 Подключиться!')).add(KeyboardButton('О проекте'))

reg_platform_kb = ReplyKeyboardMarkup(resize_keyboard=True)    
reg_platform_kb.add(KeyboardButton('\U0001F4F1 Смартфон')).insert(KeyboardButton('\U0001F4BB ПК')).add(KeyboardButton('Отмена'))
...
reg_ref_promo_kb.add(KeyboardButton('Нет промокода')).add(KeyboardButton('Отмена'))

account_kb = ReplyKeyboardMarkup(resize_keyboard=True)
account_kb.add(KeyboardButton('\u2764\uFE0F\u200D\U0001F525 Продлить подписку!')).\
