...
async def account_configurations_request_fsm_start(message: Message):
    ...
    configs_number = await postgesql_db.get_configurations_number(await postgesql_db.get_clientID_by_telegramID(message.from_user.id))
...
async def account_ref_program_promocode(message: Message):
    ...
    ref_promo_phrase: str = await postgesql_db.get_referral_promo(message.from_user.id)
